...
void VulkanRendererContext::createSwapchain() {
    ...
    RLOG("swapchain created: %dx%d presentMode=%d compositeAlpha=%d",
        swapchainExt.width,swapchainExt.height,(int)presentMode,(int)compositeAlpha);
...
void VulkanRendererContext::createWinTexPool() {
    ...
    ci.poolSizeCount=1; ci.pPoolSizes=&ps; ci.maxSets=128;
...
void VulkanRendererContext::updateWindowContent(int64_t id, void* px, short w, short h, short stride, int, int) {
    ...
    if (!mapped) return;
        size_t dstPitch=(size_t)w*4;
        {
            ...
            if (it!=texMap.end()) {
                if (it->second.useLinear)
                    dstPitch=(size_t)it->second.imgLayout.rowPitch;
                it->second.dirty=true;
                if (it->second.stgCached && it->second.stgMem!=VK_NULL_HANDLE) {
                    VkMappedMemoryRange r{}; r.sType=VK_STRUCTURE_TYPE_MAPPED_MEMORY_RANGE;
                    r.memory=it->second.stgMem; r.offset=0; r.size=VK_WHOLE_SIZE;
                    vk_.FlushMappedMemoryRanges(device,1,&r);
                }
            }
        }
...
void VulkanRendererContext::setFilterMode(int mode) {
    ...
    for (auto& [ahb,cached]:ahbTexCache) updateDS(cached.ds, cached.view);
...
void VulkanRendererContext::setSwapRB(bool enabled) {
    ...
    if (scanoutActive.load()) {
        RLOG("setSwapRB: native swapRB requires GPU blit path (alwaysGpuBlit=%d)",
